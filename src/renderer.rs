...
pub fn render(document: &Document, options: Options) -> String { render_blocks(&document.blocks, options) }
...
fn render_blocks(blocks: &[Block], options: Options) -> String {
    ...
    for block in blocks {
        match block {
            ...
            Block::List { ordered, start, items } => render_list(&mut out, *ordered, *start, items, options),
            Block::BlockQuote(blocks) => {
                out.push_str("<blockquote>\n");
                out.push_str(&render_blocks(blocks, options));
                out.push_str("</blockquote>\n");
            }
            Block::Table { header, alignments, rows } => render_table(&mut out, header, alignments, rows, options),
...
fn render_list(out: &mut String, ordered: bool, start: u64, items: &[ListItem], options: Options) {
    ...
    for item in items {
        if options.gfm && item.checked.is_some() { out.push_str("<li class=\"task-list-item\">"); }
        else { out.push_str("<li>"); }
        if let Some(checked) = item.checked {
            out.push_str("<input type=\"checkbox\" disabled");
            if checked { out.push_str(" checked"); }
            out.push_str("> ");
        }
...
fn render_table(out: &mut String, header: &[String], alignments: &[TableAlignment], rows: &[Vec<String>], options: Options) {
    out.push_str("<table>\n<thead>\n<tr>");
    for (cell, alignment) in header.iter().zip(alignments) {
        out.push_str(&format!("<th{}>{}</th>", alignment_attr(*alignment), inline(cell, options)));
    }
    out.push_str("</tr>\n</thead>\n<tbody>\n");
    for row in rows {
        out.push_str("<tr>");
        for (cell, alignment) in row.iter().zip(alignments) {
            out.push_str(&format!("<td{}>{}</td>", alignment_attr(*alignment), inline(cell, options)));
        }
        out.push_str("</tr>\n");
    }
    out.push_str("</tbody>\n</table>\n");
