...
fn heading(line: &str) -> Option<(u8, &str)> {
    ...
    Some((
        level as u8,
        trimmed[level..].trim().trim_end_matches('#').trim(),
    ))
}
...
fn setext_heading<'a>(lines: &[Line<'a>], index: usize) -> Option<(u8, &'a str)> {
    ...
    if heading(content).is_some()
        || fence_opening(content).is_some()
        || is_rule(content)
        || list_marker(content).is_some()
        || quote_content(content).is_some()
    {
        return None;
    }
    let marker = underline.chars().next()?;
    if !matches!(marker, '=' | '-') || !underline.chars().all(|ch| ch == marker) {
        return None;
    }
...
fn list_marker(line: &str) -> Option<ListMarker<'_>> {
    ...
    Some(ListMarker {
        ordered: true,
        start: content[..digits]
            .parse()
            .expect("ASCII digits parse as an integer"),
        indent,
...
fn list(
    ...
    while i < lines.len() {
        let Some(marker) = list_marker(lines[i].text) else {
            break;
        };
        if marker.ordered != ordered || marker.indent != indent {
            break;
        }
...
fn paragraph(lines: &[Line<'_>], mut i: usize, options: Options) -> (String, usize) {
    ...
    while i < lines.len() {
        let line = lines[i].text;
        if line.trim().is_empty()
            || fence_opening(line).is_some()
            || heading(line).is_some()
            || (options.gfm && table(lines, i).is_some())
            || setext_heading(lines, i).is_some()
            || is_rule(line)
...
fn table(lines: &[Line<'_>], index: usize) -> Option<(Block, usize)> {
    ...
    while let Some(line) = lines.get(next) {
        let Some(row) = split_table_row(line.text) else {
            break;
        };
