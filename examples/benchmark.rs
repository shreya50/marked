...
fn main() {
    ...
    let compare_js = arguments
        .first()
        .is_some_and(|argument| argument == "--compare-js");
    if compare_js {
        arguments.remove(0);
    }
    ...
    assert!(
        iterations > 0 && repetitions > 0,
        "iterations and repetitions must be greater than zero"
    );
    ...
    if compare_js {
        ...
        println!(
            "🚀 {:.1}× faster",
            marked.mean_ms / (elapsed.as_secs_f64() * 1_000.0 / iterations as f64)
        );
